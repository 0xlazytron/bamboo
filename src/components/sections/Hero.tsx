...
const nftCards = [
  { title: "Bamboo Ultra Rare", gif: RARE },
  { title: "Bamboo Rare", gif: SURARE },
  { title: "Mythic Rare", gif: MYTHIC }
];

const Hero = () => {
  // const navigate = useNavigate();
  const [hoveredCard, setHoveredCard] = useState<number | null>(null);
  const [selectedCard, setSelectedCard] = useState<{ title: string, gif: string } | null>(null);
  useEffect(() => {
    if (selectedCard) {
  ...
  return (
    <div className=" flex items-center px-4 container mx-auto ">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-12 w-full mt-[60px] md:mt-20">
        <motion.div
      ...
      {selectedCard && (
        ...
        <div className="fixed inset-0  flex items-center justify-center z-50 glass2">
          <div className=" p-6 rounded-lg max-w-lg relative">
            ...
            <img src={selectedCard.gif} alt={selectedCard.title} className="w-full h-auto rounded-lg" />
