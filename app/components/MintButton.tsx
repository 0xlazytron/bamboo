...
const mintClick = async (
  ...
  if (process.env.NEXT_PUBLIC_BUYMARKBEER === "false") {
    buyBeer = false;
    console.log("The Creator does not want to pay for Gushpoor 👑 beer 😒");
  }
        ...
        <Tooltip label={buttonGuard.tooltip} aria-label="Mint button">
          ...
          >
            {guardList.find((elem) => elem.label === buttonGuard.label)?.minting ? (
              // Minting state: Show loader
              <>
                <div className="w-5 h-5 border-2 border-black border-t-transparent rounded-full animate-spin mr-2"></div>
                {guardList.find((elem) => elem.label === buttonGuard.label)?.loadingText || "Minting..."}
              </>
            ) : (
