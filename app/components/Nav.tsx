                ...
                >
                    <div className="container mx-auto px-6 py-4 flex flex-col space-y-4">
                        ...
                        <div className="flex items-center space-x-4 py-2">
                            ...
                            <a href="https://discord.gg/yX6c2tuPVU" className="hover:text-purple-400 transition-colors text-white">
                        ...
                        <div className="relative text-white">
                            <button
                                onClick={() => connected && setIsDropdownOpen(!isDropdownOpen)} // Only toggle dropdown if wallet is connected
                                className="btn-gradient-primary px-4 py-2 rounded-lg flex items-center space-x-2 w-full justify-center text-white"
                            >
                                {connected ? (
                                    <>
                                        <span className="text-white">{`${publicKey?.toBase58().slice(0, 4)}...${publicKey?.toBase58().slice(-4)}`}</span>
                                        <span className="text-white">{solBalance.toFixed(4)} SOL</span>
                                    </>
                                ) : (
                                    <>
                                        <Wallet className="h-5 w-5 text-white" />
                                        <span className="text-white" onClick={handleConnectWallet}>Connect Wallet</span>
                                    </>
                                )}
                            </button>
                            {connected && isDropdownOpen && (
                                <div
                                    className="absolute right-0 mt-2 w-48 rounded-lg overflow-hidden shadow-lg z-10 text-white"
                                    style={{
                                        background:
                                            "linear-gradient(135deg, #ff7eb3, #ff758c, #ff6f65)",
                                    }}
                                >
                                    <button
                                        onClick={handleCopyAddress}
                                        className="w-full text-left px-4 py-2 hover:bg-opacity-80 transition-colors flex items-center space-x-2 text-white"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-2 hover:bg-opacity-80 transition-colors flex items-center space-x-2 text-white"
                                    >
                                        <DisconnectIcon className="h-4 w-4 text-white" />
